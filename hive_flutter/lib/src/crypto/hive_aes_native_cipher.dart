...
class HiveAesNativeCipher extends HiveAesCipher {
  ...
  @override
  Future<ParallelEncryption> encryptParallel(
    Uint8List inp,
    int inpOff,
    int inpLength,
    ...
    /// return the supposed operation
    Future<Uint8List> operation() async {
      final algorithm = _algorithm;
      ...
      if (outOffset > out.length) {
        ...
        out = Uint8List(outOffset)
          ..setRange(0, previousOut.length, previousOut);
      }
    ...
    return ParallelEncryption(
        outputLength: inpLength + 27, operation: operation.call());
  }
}
